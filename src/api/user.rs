...
pub async fn signin(Json(login_form): Json<LoginForm>, pool: Extension<Pool>) -> Response {
...
pub async fn register(
    ...
    match user::register(user, &pool, enforcer).await {
        Ok(message) => Json(ResponseBody::new(&message, constants::EMPTY)).into_response(),
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn delete_user(Path(id): Path<String>, pool: Extension<Pool>) -> Response {
    match user::delete_user(id.parse::<i32>().unwrap(), &pool) {
        Ok(delete) => Json(ResponseBody::new(constants::MESSAGE_OK, delete)).into_response(),
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn get_all_user(pool: Extension<Pool>) -> Response {
    match user::get_all_user(&pool) {
        Ok(users) => Json(ResponseBody::new(constants::MESSAGE_OK, users)).into_response(),
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            constants::MESSAGE_CAN_NOT_FETCH_DATA.to_string(),
        )
